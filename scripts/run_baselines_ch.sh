...
--evalb $EVALB \
--evalb_param $EVALB_PARAM > $LOGS/ctb.enriched.emlp.cores=1.nogpu=$USE_GPU.$TEST_NAME.log 2>&1
...
--retagger \
--evalb $EVALB \
--evalb_param $EVALB_PARAM > $LOGS/ctb.retagger.emlp.cores=1.nogpu=$USE_GPU.$TEST_NAME.log 2>&1
